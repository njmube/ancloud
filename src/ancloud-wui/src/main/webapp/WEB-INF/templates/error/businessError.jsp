<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
<title><spring:message code="err.sys.0057" /></title>
<link type="text/css"
	href="${pageContext.request.contextPath}/resources/media/css/bootstrap.css"
	rel="stylesheet" />

<script type="text/javascript">
var messageString = '<spring:message code="err.sys.0057"/>';
CONTEXT_PATH = "<%=request.getContextPath()%>
	";
</script>

<script type="text/javascript"
	src="${pageContext.request.contextPath}/resources/media/js/jquery-1.11.2.min.js"></script>
<script type="text/javascript"
	src="${pageContext.request.contextPath}/resources/app/common/javascript/errorPage.js"></script>
<style type="text/css">
body {
	background-image:
		url(data:image/png;base64,);
}

.error-template {
	padding: 40px 15px;
	text-align: center;
}

.error-actions {
	margin-top: 15px;
	margin-bottom: 15px;
}

.error-actions .btn {
	margin-right: 10px;
}
</style>
</head>
<body>
	<div class="container">
		<div class="row">
			<div class="col-md-12">
				<div class="error-template">
					<h1 id="header">Business Error!</h1>
					<div class="error-details">
						<div class="error">
							<c:choose>
								<c:when test="${empty exceptionCode}">
									<spring:message code="err.sys.0057" />
								</c:when>
								<c:otherwise>
									<c:out value="${exceptionCode}" escapeXml="true"></c:out>
									<spring:message code="${exceptionCode}" />
								</c:otherwise>
							</c:choose>
							<t:messagesPanel />
						</div>
					</div>
					<div class="error-actions">
						<a href="javascript:void(0)" class="btn btn-primary btn-lg"
							onclick="takeMeHome();"><span
							class="glyphicon glyphicon-home"></span> <spring:message
								code="sc.sys.js.0032" />bu </a>
					</div>
				</div>
			</div>
		</div>
	</div>
</body>
</html>